# ==========================================================
# RD vs AGE — PATIENTS ONLY — ln(Age) Mixed-Effects Model
#
# Description:
#   This script models the relationship between Radial
#   Diffusivity (RD) in the Optic Radiations and age in a
...
#   - RD_vs_Age_Patients_LogAge.png    : Marginal RD trajectories by NF1 status
#   - Residuals_Patients_LogAge_RD.png : Conditional residuals by NF1 status
#
# Requirements:
...
df <- read_csv("RD_JM_updated.csv", show_col_types = FALSE) %>%
  ...
  select(patient_id, Age, Sex, Group, NF1_bin,
         RD, Diagnosis_age, ln_age, ln_diag_age)
...
fit_log <- lme(
  RD ~ NF1_bin * ln_age + Sex + ln_diag_age,
...
ggsave(
  filename = "RD_vs_Age_Patients_LogAge.png",
  plot     = p_rd,
  width    = 8, height = 6, dpi = 300
)
cat("\nSaved: RD_vs_Age_Patients_LogAge.png\n")
...
ggsave(
  filename = "Residuals_Patients_LogAge_RD.png",
  plot     = p_resid,
  width    = 8, height = 6, dpi = 300
)
cat("Saved: Residuals_Patients_LogAge_RD.png\n")
