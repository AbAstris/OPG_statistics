# ==========================================================
# VA (AVERAGE) vs ln(Time Since Diagnosis) — PATIENTS ONLY
# ln(Time Since Diagnosis) Mixed-Effects Model
#
# Description:
#   This script models the relationship between mean visual
#   acuity (VA, average of both eyes) and time since diagnosis
#   in a patients-only cohort, using a linear mixed-effects
#   model with a log-transformed time since diagnosis term.
...
#   - VA_avg_vs_TimeSinceDx_LogAge.png    : Marginal VA trajectories by NF1 status
#   - Residuals_VA_avg_TimeSinceDx_LogAge.png : Conditional residuals by NF1 status
#
# Requirements:
...
# Input the FA file, shown here as FA_file.csv, used to look up Sex:
...
df <- read_csv("VA_years_JM.csv", show_col_types = FALSE) %>%
  ...
  filter(!is.na(VA_average)) %>%
  filter(!is.na(Age), Age >= 0, Age <= 20) %>%
  ...
  select(patient_id, Age, Sex, Group, NF1_bin,
         VA_average, Diagnosis_age, ln_diag_age,
...
fit_log <- lme(
  VA_average ~ NF1_bin * ln_time_since_dx + Sex + ln_diag_age,
...
p_va <- ggplot() +
  geom_point(
    data  = df,
    aes(x = time_since_dx, y = VA_average, colour = Group),
    alpha = 0.4, size = 1.5
  ) +
  geom_ribbon(
    data  = pred_df,
...
ggsave(
  filename = "VA_avg_vs_TimeSinceDx.png",
  plot     = p_va,
  width    = 8, height = 6, dpi = 300
)
cat("\nSaved: VA_avg_vs_TimeSinceDx.png\n")
...
df$residual <- df$VA_average - df$fitted
...
ggsave(
  filename = "Residuals_VA_avg_TimeSinceDx.png",
  plot     = p_resid,
  width    = 8, height = 6, dpi = 300
)
cat("Saved: Residuals_VA_avg_TimeSinceDx.png\n")