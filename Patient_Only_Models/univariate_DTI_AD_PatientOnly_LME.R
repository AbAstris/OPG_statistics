# ==========================================================
# AD vs AGE — PATIENTS ONLY — ln(Age) Mixed-Effects Model
#
# Description:
#   This script models the relationship between Axial
#   Diffusivity (AD) in the Optic Radiations and age in a
...
#   - AD_vs_Age_Patients_LogAge.png    : Marginal AD trajectories by NF1 status
#   - Residuals_Patients_LogAge_AD.png : Conditional residuals by NF1 status
#
# Requirements:
...
df <- read_csv("AD_JM_updated.csv", show_col_types = FALSE) %>%
  ...
  select(patient_id, Age, Sex, Group, NF1_bin,
         AD, Diagnosis_age, ln_age, ln_diag_age)
...
fit_log <- lme(
  AD ~ NF1_bin * ln_age + Sex + ln_diag_age,
...
# ticks display as plain numbers e.g. 1.2, 1.4 etc.
# The axis label carries the (x 10^-3 mm^2/s) unit explicitly.
...
ggsave(
  filename = "AD_vs_Age_Patients_LogAge.png",
  plot     = p_ad,
  width    = 8, height = 6, dpi = 300
)
cat("\nSaved: AD_vs_Age_Patients_LogAge.png\n")
...
ggsave(
  filename = "Residuals_Patients_LogAge_AD.png",
  plot     = p_resid,
  width    = 8, height = 6, dpi = 300
)
cat("Saved: Residuals_Patients_LogAge_AD.png\n")
