# ==========================================================
# RD vs AGE — ln(Age) Mixed-Effects Model
#
# Description:
#   This script models the relationship between Radial
#   Diffusivity (RD) in the Optic Radiations and age using a
...
#
# Inputs:
#   - RD_file.csv         : Patient DTI data (see column layout below)
#   - Control_RD_file.csv : Control DTI data (see column layout below)
#   Rename these to match your actual file names.
#
# Outputs:
#   - Console: subject counts and model summary
#   - RD_vs_Age_LogAge.png    : Marginal RD trajectories per group
#   - Residuals_LogAge_RD.png : Conditional residuals per group
#
# Requirements:
...
# Input the patient DTI file, shown here as RD_file.csv with column layout:
# ├── Subject                    (patient ID — numeric or string)
# ├── Weighted Avg RD Combined   (RD measurement — numeric, e.g. 0.00045)
# ├── Age                        (age in years — numeric, 0–20)
# ├── Sex                        (categorical — "F" or "M")
# └── NF1                        (categorical — "Y" or blank/NA)

# Input the control DTI file, shown here as Control_RD_file.csv
...
# ── Patients ───────────────────────────────────────────────
patients <- read_csv("RD_JM_updated.csv", show_col_types = FALSE) %>%
  rename(RD = `Weighted Avg RD Combined`) %>%
  mutate(
    patient_id = as.character(Subject),
    NF1        = ifelse(is.na(NF1) | NF1 == "", "N", NF1),
    Group      = ifelse(NF1 == "Y", "Patient_NF1", "Patient_nonNF1")
  ) %>%
  filter(!is.na(Age), !is.na(RD), Age >= 0, Age <= 20) %>%
  group_by(patient_id) %>%
  filter(n() >= 2) %>%
  ungroup() %>%
  select(patient_id, Age, Sex, Group, RD)

# ── Controls ───────────────────────────────────────────────
controls <- read_csv("../../control_data/Controls_RD_OR_hdbet.csv",
...
# RD values are multiplied by 1e4 in the plot so that axis
# ticks display as plain numbers e.g. 4.0, 5.0 etc.
...
p_rd <- ggplot() +
  geom_point(
    data  = df,
    aes(x = Age, y = RD * 1e4, colour = Group),
    alpha = 0.4, size = 1.5
  ) +
  geom_ribbon(
    data  = pred_df,
  ...
  labs(
    x      = "Age (years)",
    y      = expression(RD~"in Optic Radiations"~"("*x*" 10"^{-4}~mm^2*"/s)"),
    colour = "Group",
    fill   = "Group"
  ) +
...
ggsave(
  filename = "RD_vs_Age_LogAge.png",
  plot     = p_rd,
  width    = 8, height = 6, dpi = 300
)
cat("\nSaved: RD_vs_Age_LogAge.png\n")
...
ggsave(
  filename = "Residuals_LogAge_RD.png",
  plot     = p_resid,
  width    = 8, height = 6, dpi = 300
)
cat("Saved: Residuals_LogAge_RD.png\n")
