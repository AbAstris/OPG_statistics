# ==========================================================
# FA vs AGE — PATIENTS ONLY — ln(Age) Mixed-Effects Model
#
# Description:
#   This script models the relationship between Fractional
#   Anisotropy (FA) in the Optic Radiations and age in a
...
#   - FA_vs_Age_Patients_LogAge.png    : Marginal FA trajectories by NF1 status
#   - Residuals_Patients_LogAge_FA.png : Conditional residuals by NF1 status
#
# Requirements:
...
# Input the patient DTI file, shown here as FA_file.csv with column layout:
# ├── Subject              (patient ID — numeric or string)
# ├── Weighted Avg FA Combined (FA measurement — numeric, e.g. 0.45)
...
df <- read_csv("FA_JM_updated.csv", show_col_types = FALSE) %>%
  ...
  select(patient_id, Age, Sex, Group, NF1_bin,
         FA, Diagnosis_age, ln_age, ln_diag_age)
...
fit_log <- lme(
  FA ~ NF1_bin * ln_age + Sex + ln_diag_age,
...
ggsave(
  filename = "FA_vs_Age_Patients_LogAge.png",
  plot     = p_fa,
  width    = 8, height = 6, dpi = 300
)
cat("\nSaved: FA_vs_Age_Patients_LogAge.png\n")
...
ggsave(
  filename = "Residuals_Patients_LogAge_FA.png",
  plot     = p_resid,
  width    = 8, height = 6, dpi = 300
)
cat("Saved: Residuals_Patients_LogAge_FA.png\n")